/* SocialBot board v2 — self-contained, no fetch */
const STATE_KEY = "socialbot-board-v2-sep7-2026";
...
function dayCardHTML(day, st) {
  ...
  const previewClass = isPlaceholder(li) ? "draft-preview placeholder" : "draft-preview";
  ...
  const liOpen = st.openEditor === "linkedin" ? "open" : "";
  const xOpen = st.openEditor === "x" ? "open" : "";
  ...
  <article class="${cardCls}" data-id="${day.id}">
    ...
    <div class="draft-block">
      <div class="draft-label">Draft · LinkedIn (primary)</div>
      <pre class="${previewClass}">${escapeHtml(li)}</pre>
    </div>

    <div class="channel-bar">
      <button type="button" class="channel-btn ${liOpen}" data-ch="linkedin">LinkedIn<span class="ch-tag">edit</span></button>
      <button type="button" class="channel-btn ${xOpen}" data-ch="x">X<span class="ch-tag">edit</span></button>
    </div>

    <div class="editor-pane ${liOpen}" data-editor="linkedin">
      <h4>LinkedIn copy</h4>
    ...
    <div class="editor-pane ${xOpen}" data-editor="x">
      <h4>X copy</h4>
...
function renderDecisions(state) {
  ...
  if (!lines.length) {
    el.innerHTML = `<p class="empty">No edits yet. Expand LinkedIn/X to edit, approve/kill a day, or promote from the queue.</p>`;
    return;
  }
  ...
  document.querySelectorAll(".day").forEach(card => {
    ...
    card.querySelectorAll("[data-save]").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        // refresh preview without closing editor
        const preview = card.querySelector(".draft-preview");
        if (field === "linkedin" && preview) {
          preview.textContent = state.days[id].linkedin;
          preview.classList.toggle("placeholder", isPlaceholder(state.days[id].linkedin));
        }
        renderDecisions(state);
      });
    });
    ...
    card.querySelectorAll("[data-act]").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        // grab any open textarea notes? we don't have a note field UI — use decision only
        // Keep a lightweight note via prompt only if Save note
        if (act === "note") {
